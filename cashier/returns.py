...
def _get_supervisor_pin_hashes() -> list[str]:
    q = ("SELECT pin_hash FROM users "
         "WHERE role IN ('Admin','Supervisor') AND pin_hash IS NOT NULL")
    df = db.fetch_data(q)
    return [h for h in df["pin_hash"].tolist() if h]


def _pin_ok(pin_try: str) -> bool:
    for h in _get_supervisor_pin_hashes():
        if verify_pin(pin_try, h):
            return True
    return False
...
def _clear_return_state():
    for k in [
        "return_sale_id", "return_sale_details", "return_sale_items",
        "full_return_attempt", "pending_return", "pin_error"
    ]:
...
def display_return_tab() -> None:
    ...
    if "pending_return" in ss:
        ...
        # ---------- human-readable list of the items being returned
        # We try to resolve an item name from the sale-items DF cached earlier.
        name_map = {}
        if "return_sale_items" in ss:
            name_map = {
                int(r["itemid"]):
                    r.get("itemname") or r.get("itemnameenglish") or f"Item {r['itemid']}"
        ...
        st.info(
            f"### Confirm **{pending['mode']}** for original Sale ID **{pending['orig_id']}**"
        )
        ...
        if c_ok.button("✅ Confirm return", key="pin_ok_btn"):
            if pin and _pin_ok(pin):
                # … (unchanged  ⟶  finalise sale) …
        ...
        if c_cancel.button("❌ Cancel", key="pin_cancel_btn"):
            _clear_return_state()
        return
    ...
    ret_df = cashier_handler.fetch_data(
        """
        SELECT itemid, SUM(ABS(quantity)) AS returned_qty
          FROM salesitems
         WHERE saleid IN (
    ...
    already_map = (
        ret_df.set_index("itemid")["returned_qty"].to_dict()
        if not ret_df.empty else {}
    )
    ...
    if st.button("Return entire remaining amount"):
        ...
        for _, row in sale_items_df.iterrows():
            ...
            name    = row.get("itemname") or row.get("itemnameenglish") or "Item"   # ← FIX
            details.append((name, sold, already, allowed))
            if already: partial = True
            if allowed:
                cart_items.append(
                    {"itemid": itemid, "quantity": -allowed,
        ...
        if partial and not ss.get("full_return_attempt"):
            st.warning("Some items were already returned – press again to confirm the remaining items.")
            st.dataframe(
                pd.DataFrame(details,
                             columns=["Item", "Sold", "Returned", "Allowed"]),
        ...
        ss.pending_return = dict(
            ...
            items=cart_items,
            disc_rate=float(ss.return_sale_details.iloc[0].get("discountrate", 0.0)),
    ...
    for idx, row in sale_items_df.iterrows():
        ...
        qty = c3.number_input("", 0, allowed, allowed, 1, key=f"ret_qty_{idx}")
        return_rows.append((chk, qty, itemid, row["unitprice"]))

    if st.button("Return selected items"):
        cart_items = [
            {"itemid": iid, "quantity": -q, "sellingprice": float(price)}
